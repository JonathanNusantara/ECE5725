...
os.putenv('SDL_FBDEV', '/dev/fb0') # Display on PiTFT, run fb1 from pitft if have hdmi connected. If hdmi not connected use fb0
...
# lower_hand = (113,104,8)
# upper_hand = (128,202,96)
# # Detect based on tennis ball color
# lower_ball = (0, 66, 41)
# upper_ball = (31, 168, 215)
...
def set_speed():
    ...
    if dist_calc > 15:
        ...
        # left_speed = 10 + (dist_l - 100) + (dist_l - dist_l_pre)
        # right_speed = 10 + (dist_r - 100) + (dist_r - dist_r_pre)
        

        # right_speed = 40 + 0.1 * (dist_l - 150) + 0.1 * (dist_l - dist_l_pre)
        # left_speed = 40 + 0.1 * (dist_r - 150) + 0.1 * (dist_r - dist_r_pre)
    ...
    elif dist_calc > 10: # between 10 and 15, re-center only
        left.ChangeDutyCycle(20)
        right.ChangeDutyCycle(20)
        if dist_l + 20 < dist_r: # Clockwise
            GPIO.output(5, 1)
            GPIO.output(6, 0)
            GPIO.output(12, 0)
            GPIO.output(16, 1)
        elif dist_r + 20 < dist_l: # Counter-clockwise
            GPIO.output(5, 0)
            GPIO.output(6, 1)
            GPIO.output(12, 1)
            GPIO.output(16, 0)
...
while run:
    ...
    if (time.time() - start_time_prog) > 90: # Timeout
        run = False
