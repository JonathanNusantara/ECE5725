...
#os.putenv('SDL_FBDEV', '/dev/fb1') # Display on PiTFT, run fb1 from pitft if have hdmi connected
...
GPIO.output(20, 0) # Set BI1 to low
...
def ball_track():
    ...
    circles = cv2.HoughCircles(mask_ball,cv2.HOUGH_GRADIENT,2,120,param1=100,param2=50,minRadius=2,maxRadius=0)     
    ...
    if circles is not None:
        ...
        if ball_detected == 0 and ball_verify == 0:
            ...
            ball_verify = 1
        if ball_verify == 1 and (time.time() - ball_first_detected) > 0.5: # Verification after .5 second passes, ball still detected
            ball_detected = 1
            ball_verify = 0
        if ball_detected == 1:    
        ...
        if ball_detected == 1 and ball_lost_time > 1: # If ball has been missing for > 2 seconds
            print("rotate")
...
# Glove HSV range moving
lower_hand = (100,116,0)
upper_hand = (122,170,86)
# Detect based on tennis ball color
lower_ball = (23, 85, 99)
upper_ball = (39, 166, 218)
...
def set_speed():
    ...
    left_speed = 50 + 0.15 * (dist_l - 120) #+ 0.2 * (dist_l - dist_l_pre)
    right_speed = 50 + 0.1 * (dist_r - 120) #+ 0.2 * (dist_r - dist_r_pre)
    if left_speed > 100:
        left_speed = 100
    elif left_speed < 0:
        left_speed = 0
...
while run:
    ...
    # ONLY DISPLAY WHEN RUNNING IN STARTX, OR PROGRAM WILL CRASH
    ...
    if (time.time() - start_time_prog) > 100: # Timeout
        run = False
